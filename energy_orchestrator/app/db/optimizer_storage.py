    ...
    try:
        with Session(engine) as session:
            result_record = OptimizerResult(
                ...
                complete_feature_config_json=json.dumps(result.complete_feature_config) if result.complete_feature_config else None,
...
def _result_to_dict(result: OptimizerResult) -> dict:
    ...
    return {
        ...
        "complete_feature_config": json.loads(result.complete_feature_config_json) if result.complete_feature_config_json else None,
        "val_mape_pct": result.val_mape_pct,
        "val_mae_kwh": result.val_mae_kwh,
        "val_r2": result.val_r2,
        "train_samples": result.train_samples,
